...
#define LOCTEXT_NAMESPACE "SimpleMGFGameFeatures"
...
void UGameFeatureAction_AddAbilities::AddAdditionalAssetBundleData(FAssetBundleData& AssetBundleData)
{
	if (UAssetManager::IsValid())
	{
		...
		{
			for (const FSimpleMGFGameAbilityMapping& Ability : Entry.GrantedAbilities)
			{
			...
			for (const FSimpleMGFGameAttributesMapping& Attributes : Entry.GrantedAttributes)
			{
...
{
	UE_LOG(LogSimpleMGFGame, Error, TEXT("Added ability"), *Actor->GetPathName());
	if (UAbilitySystemComponent* AbilitySystemComponent = FindOrAddComponentForActor<UAbilitySystemComponent>(Actor, AbilitiesEntry))
	{
		...
		for (const FSimpleMGFGameAbilityMapping& Ability : AbilitiesEntry.GrantedAbilities)
		{
			if (!Ability.AbilityType.IsNull())
			{
				...
				if (!Ability.InputAction.IsNull())
				{
					...
					{
						 UE_LOG(LogSimpleMGFGame, Error, TEXT("Failed to find/add an ability input binding component to '%s' -- are you sure it's a pawn class?"), *Actor->GetPathName());
					}
				}
		...
		for (const FSimpleMGFGameAttributesMapping& Attributes : AbilitiesEntry.GrantedAttributes)
		{
			if (!Attributes.AttributeSetType.IsNull())
			{
				TSubclassOf<USimpleMGFGameAbilityAttributeSet> SetType = Attributes.AttributeSetType.LoadSynchronous();
				if (SetType)
				{
	...
	{
		UE_LOG(LogSimpleMGFGame, Error, TEXT("Failed to find/add an ability component to '%s'. Abilities will not be granted."), *Actor->GetPathName());
	}
}
